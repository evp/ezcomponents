                                   ...
                                   ezcImageThumbnailFilters
{
    ...
    public function scaleWidth( $width, $direction )
    {
        ...
        {
            case self::SCALE_BOTH:
                $newDim = ['x' => $width, 'y' => $width / $oldDim['x'] * $oldDim['y']];
                break;
            case self::SCALE_UP:
                $newDim = ['x' => max( $width, $oldDim['x'] ), 'y' => $width > $oldDim['x'] ? round( $width / $oldDim['x'] * $oldDim['y'] ) : $oldDim['y']];
                break;
            case self::SCALE_DOWN:
                $newDim = ['x' => min( $width, $oldDim['x'] ), 'y' => $width < $oldDim['x'] ? round( $width / $oldDim['x'] * $oldDim['y'] ) : $oldDim['y']];
                break;
            default:
                throw new ezcBaseValueException( 'direction', $direction, 'self::SCALE_BOTH, self::SCALE_UP, self::SCALE_DOWN' );
                break;
    ...
    public function scaleHeight( $height, $direction )
    {
        ...
        {
            case self::SCALE_BOTH:
                $newDim = ['x' => $height / $oldDim['y'] * $oldDim['x'], 'y' => $height];
                break;
            case self::SCALE_UP:
                $newDim = ['x' => $height > $oldDim['y'] ? round( $height / $oldDim['y'] * $oldDim['x'] ) : $oldDim['x'], 'y' => max( $height, $oldDim['y'] )];
                break;
            case self::SCALE_DOWN:
                $newDim = ['x' => $height < $oldDim['y'] ? round( $height / $oldDim['y'] * $oldDim['x'] ) : $oldDim['x'], 'y' => min( $height, $oldDim['y'] )];
                break;
            default:
                throw new ezcBaseValueException( 'direction', $direction, 'self::SCALE_BOTH, self::SCALE_UP, self::SCALE_DOWN' );
                break;
