...
class ezcDocumentWikiDokuwikiTokenizer extends ezcDocumentWikiTokenizer
{
    ...
    public function __construct()
    {
        $this->tokens = [
            ...
            ['class' => 'ezcDocumentWikiTextLineToken', 'match' => '(\\A(?P<match>\\n<nowiki>\\n(?P<value>.+)\\n</nowiki>)\\n)SUsi'],
            ...
            ['class' => 'ezcDocumentWikiSuperscriptToken', 'match' => '(\\A(?P<value></?sup>))Si'],
            ['class' => 'ezcDocumentWikiSubscriptToken', 'match' => '(\\A(?P<value></?sub>))Si'],
            ['class' => 'ezcDocumentWikiUnderlineToken', 'match' => '(\\A(?P<value>__))S'],
            ['class' => 'ezcDocumentWikiDeletedToken', 'match' => '(\\A(?P<value></?del>))Si'],
            ['class' => 'ezcDocumentWikiInlineLiteralToken', 'match' => '(\\A<nowiki>(?P<value>.*)</nowiki>)SUi'],
            ['class' => 'ezcDocumentWikiTextLineToken', 'match' => '(\\A%%(?P<value>.*)%%)SUi'],
            ['class' => 'ezcDocumentWikiLineBreakToken', 'match' => '(\\A(?P<match>(?P<value>\\\\\\\\))(?:' . self::WHITESPACE_CHARS . '|\\n))S'],
            ...
            ['class' => 'ezcDocumentWikiExternalLinkToken', 'match' => '(\\A
                        (?P<match>
                            (?P<value>
                                # Match common URLs
                                [a-z]+://\S+? |
            ...
            ['class' => 'ezcDocumentWikiInterWikiLinkToken', 'match' => '(\\A(?P<value>([A-Za-z]+)>[^\\]|]+))S'],
            ['class' => 'ezcDocumentWikiImageStartToken', 'match' => '(\\A(?P<value>\\{\\{))S'],
            ['class' => 'ezcDocumentWikiImageEndToken', 'match' => '(\\A(?P<value>\\}\\}))S'],
            ['class' => 'ezcDocumentWikiFootnoteStartToken', 'match' => '(\\A(?P<value>\\(\\())S'],
            ['class' => 'ezcDocumentWikiFootnoteEndToken', 'match' => '(\\A(?P<value>\\)\\)))S'],
            ['class' => 'ezcDocumentWikiTableHeaderToken', 'match' => '(\\A(?P<value>\\^))S'],
            ['class' => 'ezcDocumentWikiPluginToken', 'match' => '(\\A(?P<value><([a-zA-Z]+).*?</\\2>))Ss'],
            // Match text except
    ...
    protected function parsePluginContents( ezcDocumentWikiPluginToken $plugin )
    {
        ...
        if ( preg_match( '(^\\s*<(?P<type>[a-zA-Z]+)(?:\\s+(?P<params>[^>]+))?>(?P<content>.*?)\\s*</\\1>\\s*)si', $plugin->content, $match ) )
        {
            ...
            $plugin->parameters = isset( $match['params'] ) && $match['params'] ? [$match['params']] : [];
